 ...
 *
 * The website only ever reads this record — it is edited in the admin panel —
 * so the context exposes the values and whether they have arrived, nothing more.
 */
export const OrganizationProvider = ({ children }) => {
  ...
  useEffect(() => {
    api
      .get('/settings')
      // A failure leaves the blank record in place: every component renders
      // nothing for a blank value, so the page degrades rather than breaking.
      .then((res) => setOrganization(normalizeOrganization(res.data)))
      .catch((err) => console.error("Error loading organization settings:", err))
      .finally(() => setLoading(false));
  }, []);

  const value = useMemo(() => ({ organization, loading }), [organization, loading]);
